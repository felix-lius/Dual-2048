# 风格参考手册 · 《Double 2048: Dual Challenge》最终上线版

> **用途**：把本游戏 v1.0.0（已发布 / 上线）的**最终美术、音频、设计基调**整理成一份可复用参考，供你下一款「数字方块合并」类游戏照抄，保持同一工作室出品的一致风格。
> **汇编**：主理人游承峰编排；三节分别由 文策渊（设计基调）、林绘澄（视觉）、阮和鸣（音频）基于**最终上线代码**整理。
> **重要**：本手册数值一律以**上线实现（as-built）**为准。早期 `design/art/art-bible.md` 与 `design/audio/audio-bible.md` 仍是草案，含大量「待拍板」项；**凡与本手册冲突，以本手册为准**。
> **配套原始文档**：设计 GDD `design/gdd/dual-2048-gdd.md`、美术圣经 `design/art/art-bible.md`、音频圣经 `design/audio/audio-bible.md` 为设计阶段文档；本手册是它们的「上线实况快照」。

---

## 0. 速览：风格签名与最终上线事实（已核实）

**一句话风格签名**

> calm-but-playful 的「糖果卡通风」——画面明亮圆润暖洋洋，让高强度数字合并看起来轻松愉快；听觉上零音频文件、移动音是一记极轻的下扫「嗖」、音乐默认安静。

**五条最关键的最终上线事实**（与早期草案不同，移植时务必注意）：
...
**可以自由更换**：玩法单元数量（单盘/三盘/6×6…）· 张力来源（限步/重力/属性）· 辅助种类（炸弹/换位/洗牌，但守支柱3）· 胜利目标 · 难度分档 · 具体色值（只要仍明亮低饱和暖调糖果感）· 变现点位（但必须玩家主动选、看完才给、不可用就隐藏）。
...
> 这是一份「照抄配方」。所有色值、字号、圆角、动画时长都是**已上线跑通的实测值**。做下一款数字合并游戏时，把色板/字体/圆角/动画表直接搬进新项目的 `theme.js`，视觉风格就自动一致了。
...
> 全文数值均从上线代码 `src/audio.js`（已核对与发布包逐字节一致）逐行核出。一句话情绪：**「甜甜的糖、暖暖的光，轻轻摇摆不抢戏。」**
...
| 调性 | **C 大调 / G 大调，五声音阶** |
| 上线音阶 | `C4 E4 G4 A4 C5` = `[261.63, 329.63, 392.00, 440.00, 523.25]` Hz |
| 速度 | 目标 **88–108 BPM**（⚠️ 上线步长 380ms≈79BPM 偏慢，**新游戏建议 310ms≈97BPM**） |
...
| 草案事件 | 上线状态 | 说明 |
| --- | --- | --- |
| **`merge`（合并音）** | ❌ **已下线** | ITER-V12-001 移除；`src/audio.js` 无 `case 'merge'`，全项目零调用点，调用即静默 |
...
**移动音 swipe whoosh（上线定稿）**——拿一张纸从桌面轻扫过：无音高、一股气声、从明亮迅速变低沉、一眨眼、极小声。三步：① 造 110ms 白噪声 → ② 带通滤波中心 1800→450Hz 下扫（Q=1.3，产生运动感）→ ③ 增益包络 6ms 升到 0.038 再指数衰减。一行代码：`this._noise({ dur: 0.11, peak: 0.038, band: 1800, q: 1.3, sweepTo: 450 });`。**调音纪律**：嫌听不清只调 `peak`（轻 0.026 / 标准 0.038 / 亮 0.052），**永远不要为提高 `band` 让它变刺耳**。
...
【音频】   零文件·运行时合成   移动音 _noise{band1800→450,Q1.3,110ms,peak0.038}
         ...
         广告=Master→0 整体静音（非duck）   合并音已下线(单盘新游戏建议加回)
...
*本手册由主理人游承峰汇编自《Double 2048: Dual Challenge》v1.0.0 最终上线代码（as-built），未改动任何现有文件。三节分别由 文策渊 / 林绘澄 / 阮和鸣 整理。凡标 ⚠️ 处为「草案有、成品无」或「已上线但建议新游戏改进」，可据以决定继承或修正；凡与早期 `art-bible.md` / `audio-bible.md` 冲突，以本手册为准。*
